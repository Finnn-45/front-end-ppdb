...
const PageFormUpload: React.FC = () => {
  ...
  const housePhotoTypes = [
    ...
    { name: "belakang", label: "Tampak Belakang", example: "/examples/belakang.jpg" },
    { name: "samping", label: "Tampak Samping", example: "/examples/samping.jpg" },
    { name: "ruangtamu", label: "Ruang Tamu", example: "/examples/ruangtamu.jpg" },
    { name: "kamar", label: "Kamar Tidur", example: "/examples/kamar.jpg" },
    { name: "dapur", label: "Dapur / Kamar Mandi", example: "/examples/dapur.jpg" },
  ];
    ...
    <>
      ...
      <div className="w-full max-w-6xl mx-auto bg-gray-50 rounded-xl p-6 sm:p-10 shadow-sm animate__animated animate__fadeInUp animate__slow mt-6">
            ...
            {[
              ...
              <React.Fragment key={idx}>
                <div className="flex flex-col items-center">
                  <div className={`w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center rounded-full font-semibold ${item.step === 7 ? "bg-[#1E3A8A] text-white" : "bg-gray-300 text-gray-600"}`}>
                    {item.step}
                  </div>
                  <p className={`mt-1 text-xs sm:text-sm font-medium text-center ${item.step === 7 ? "text-[#1E3A8A]" : "text-gray-500"}`}>
                    {item.label}
                  </p>
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
          {requiredFiles.map(f => (
            <div key={f.name} className={`${f.name==="bpjs" ? "sm:col-span-2 flex justify-center" : ""}`}>
              <div className={f.name==="bpjs" ? "w-full sm:w-1/2" : "w-full"}>
              ...
              <div key={photo.name} className="flex flex-col gap-4">
                ...
                <div className="border border-gray-200 rounded-xl bg-white shadow-sm hover:shadow-md transition overflow-hidden flex flex-col items-center p-3">
                  {housePreviews[photo.name] ? (
                    <img src={housePreviews[photo.name]!} alt={`Preview ${photo.label}`} className="w-full h-36 object-cover rounded-lg border border-green-200" />
                  ) : (
                  ...
                  <label htmlFor={photo.name} className={`cursor-pointer text-xs sm:text-sm flex items-center justify-center gap-2 border px-3 py-1.5 rounded-full w-full sm:w-auto text-center ${housePhotos[photo.name] ? "bg-green-100 border-green-400 text-green-700" : "bg-gray-100 border-gray-300 hover:bg-blue-50 hover:text-[#1E3A8A]"}`}>
                  ...
                  <input id={photo.name} name={photo.name} type="file" accept="image/*" className="hidden" onChange={handleHousePhotoChange} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between mt-8 gap-3">
          ...
          <button onClick={handleNext} className="w-full sm:w-auto bg-[#1E3A8A] text-white font-medium px-6 py-2 rounded-full hover:bg-[#162d66] transition">
            Selesai
          </button>
        </div>
      </div>
